...
def exercise04(more_temperatures, iot_sensor_points, a, b, c, d, e):
    ...
    # ------ Place code below here \/ \/ \/ ------
    for i in more_temperatures:
        temperatures.append(i)
    
    for j in iot_sensor_points.keys():
...
def display_menu(menu):
    ...
    user_input = input()
    chosen_menu_option = 0
    while user_input != len + 1:
    if(isinstance(user_input, int) and user_input > 0 and user_input <= len):
            print_chosen(menu, user_input)
    
    else:
        print('please retry with a valid input')
                        chosen_menu_option = user_input
                        print_menu(menu)

user_input = input()
return chosen_menu_option, len

def print_chosen(menu, input):
print(menu[input - 1])
def print_menu(menu):
counter = 1
for x in menu:
